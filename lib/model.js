...
class Model {
  ...
  static findAll(options) {
    ...
    return Promise.try(() => {
      ...
      /* Combine Association Rows into Single Row */
      if (typeof options.include === 'object' && options.include.length > 0 && options.raw) {
        const object = {}
        for (const i in results) {
          ...
          for (const key in result) {
            ...
            if (key.indexOf('.') !== -1) {
              ...
              if (!object[childTable]) {
                object[childTable] = {}
              }

              if (!object[childTable][childId]) {
                object[childTable][childId] = {}
              }

              object[childTable][childId][prop] = val
              continue
            }
      ...
      if (!include.separate) {
        return Model._findSeparate(
          results.reduce((memo, result) => {
            let associations = typeof result.get === 'function' ? result.get(include.association.as) : result;
